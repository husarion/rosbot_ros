...
> 3. When the power supply is lost, the robot loses momentum and falls by inertia. Therefore, you should hold the manipulator when the power is cut off, or call the docking node `ros2 run rosbot_moveit dock` (append `--ros-args -r __ns:=/$ROBOT_NAMESPACE` when the robot was launched in a namespace, so `dock` reaches the namespaced `move_group`) or press `RT` + `Back` buttons on gamepad.
