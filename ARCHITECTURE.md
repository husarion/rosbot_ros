...
- [launch/rosbot[_xl].yaml](rosbot_description/launch/) hosts `robot_state_publisher` (URDF → `/tmp/rosbot[_xl]_description_<ns>.urdf`) and `tf_namespace_bridge` as composable nodes in one `node_container` (`rclcpp_components/component_container_mt`) — RSP is the highest-frequency `/tf` publisher in the stack and the bridge is its direct subscriber, so composing them skips a serialize/DDS hop per transform. `tf_namespace_bridge` is a public arg (bool); its config path is an internal `let` resolved from `config_dir`, same pattern as `components_config`. Optional `joint_state_publisher` mock_joints. [launch/rviz.yaml](rosbot_description/launch/rviz.yaml).
  - **Known side effect:** loading composable nodes makes `launch_ros` spin up one unnamespaced helper node `launch_ros_<pid>` (hardcoded in `launch_ros/ros_adapters.py`, no YAML hook) to call the container's `load_node` service — it only exposes the standard empty parameter services and lives for the launch process's lifetime. Allowlisted by prefix in [test_namespace_isolation.py](rosbot_bringup/test/test_namespace_isolation.py) (`ALLOWED_GLOBAL_SERVICE_PREFIXES`). No upstream `ros2/launch_ros` issue tracks this as of 2026-08.
...
- TF namespaced via `set_remap /tf → tf` and `/tf_static → tf_static` in every launch. Global `/tf` (for nav2 / multirobot RViz) bridged by [tf_namespace_bridge](https://github.com/husarion/tf_namespace_bridge) — `tf_namespace_bridge:=True` + `namespace≠''`. `frame_filters` (glob on `child_frame_id`) controls which frames bridge; default empty = pass-through. Runs composed with `robot_state_publisher` in `rosbot_description`'s `node_container` (see §2), not as its own process. Single config, owned by `rosbot_description`: [rosbot_description/config/tf_namespace_bridge.yaml](rosbot_description/config/tf_namespace_bridge.yaml) (override via `config_dir` like any other package config).
