...
- [launch/rosbot[_xl].yaml](rosbot_description/launch/) starts `robot_state_publisher` (URDF → `/tmp/rosbot[_xl]_description_<ns>.urdf`) and `tf_namespace_bridge` as two separate processes. **Do not compose them into a `node_container`** — see CLAUDE.md §9 2026-08-12 for why (`LoadComposableNodes` races with `on_exit: shutdown` on `ros2_control_node` via a shared `launch_ros` internal singleton, corrupting shutdown for every bringup, not just crashes). `tf_namespace_bridge` is a public arg (bool); its config path is an internal `let` resolved from `config_dir`, same pattern as `components_config` — owned by this package (`config/tf_namespace_bridge.yaml`), not duplicated per caller. Optional `joint_state_publisher` mock_joints. [launch/rviz.yaml](rosbot_description/launch/rviz.yaml).
...
- [servo.launch.py](rosbot_moveit/launch/servo.launch.py) — `moveit_servo/servo_node` + `joy2servo`. `moveit_servo.yaml` uses relative `monitored_planning_scene_topic` (upstream `/planning_scene` strips namespace). Both nodes are gated by `servo_enabled` (default inherited from [manipulator.yaml](rosbot_controller/launch/manipulator.yaml)'s `arm_activate`) — `servo_node` isn't a lifecycle node, its collision loop can't be paused once started (~91% of one core, see [MANIPULATOR.md](MANIPULATOR.md)), so skipping the launch is the only way to avoid the cost when the arm isn't used.
...
- TF namespaced via `set_remap /tf → tf` and `/tf_static → tf_static` in every launch. Global `/tf` (for nav2 / multirobot RViz) bridged by [tf_namespace_bridge](https://github.com/husarion/tf_namespace_bridge) — `tf_namespace_bridge:=True` + `namespace≠''`. `frame_filters` (glob on `child_frame_id`) controls which frames bridge; default empty = pass-through. Runs as its own process, launched from `rosbot_description` alongside (not composed with) `robot_state_publisher` (see §2). Single config, owned by `rosbot_description`: [rosbot_description/config/tf_namespace_bridge.yaml](rosbot_description/config/tf_namespace_bridge.yaml) (override via `config_dir` like any other package config).
