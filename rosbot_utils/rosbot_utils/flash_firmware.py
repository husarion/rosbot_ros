...
def find_firmware_file():
    ...
    package_install_directory = ament_index_python.packages.get_package_share_directory('rosbot_utils')
    ...
    if not firmware_files:
        firmware_url = "https://github.com/husarion/rosbot_ros2_firmware/releases/download/0.8.0/firmware.bin"
...
def main(args=None):
    ...
    parser.add_argument('-p', '--port', default='/dev/ttyUSB0', help='Specify the USB port (default: /dev/ttyUSB0)')
    ...
    try:
        ...
        additional_args = ['-p', args.port, '--file', firmware_file] if args.usb else ['--file', firmware_file]
