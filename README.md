...
For ROSbot XL, you can specify a particular configuration using the launch `configuration` argument. If you are using the `manipulation` configuration, please refer to [MANIPULATOR.md](MANIPULATOR.md) for detailed instructions — including the **gamepad controls** for the OpenMANIPULATOR-X.
...
| ✅  | ❌  | `asset_server`    | Launch `husarion_asset_server`, serving this robot's `package://` meshes/URDF resources to the bridge's `get_asset` service. <br/> **_bool:_** `True`                                                                          |
| ✅  | ❌  | `battery_alert`   | ROSbot XL only. Play the low-battery alert on the on-board speaker. The speaker PCB is wired behind a USB sound card on every SBC, so without one there is no audio output at all and the node logs a single warning; `audio_device` in `rosbot_utils/config/rosbot_xl/config.yaml` overrides the auto-detected ALSA device. <br/> **_bool:_** `True` |
...
| ✅  | ✅  | `configuration` | Specify configuration packages. ROSbot XL has the full set; ROSbot 2/3 only `basic`/`pro`/`custom` (`basic` = OAK-D Lite + RPLIDAR C1 for ROSbot 3, `pro` = OAK-D Pro + RPLIDAR S2 for ROSbot 3 PRO). Packages: `basic`, `pro`, `telepresence`, `autonomy`, `manipulation`, `manipulation_pro`, `custom`. <br/> **_string:_** `basic`                                                                          |
...
| ✅  | ✅  | `joy_vel`       | The topic name to which velocity commands will be published. Defaults to the highest-priority arbitration input, so the gamepad overrides autonomy. <br/> **_string:_** `manual/cmd_vel` |
| ✅  | ❌  | `led_animation`   | ROSbot XL only. Animation published at startup. `none` publishes nothing, so the firmware idle animation shows. Switch live with `ros2 param set /<ns>/animation_publisher current_animation <name>` — the `<name>.png` and `<name>.yaml` are re-read from disk on every switch, so an animation added after startup needs no restart. <br/> **_string:_** `ready` (choices: `ready`, `navigation`, `rainbow`, `turn_off`, `none`) |
| ✅  | ❌  | `led_strip`       | ROSbot XL only. Enable or disable the LED strip. Publishing can also be gated at runtime with the `led_strip/enable` service. <br/> **_bool:_** `True` |
