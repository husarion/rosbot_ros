...
| ✅  | ❌  | `led_animation`   | ROSbot XL only. Animation published at startup. `none` publishes nothing, so the firmware idle animation shows. Switch live with `ros2 param set /<ns>/animation_publisher current_animation <name>` — the `<name>.png` and `<name>.yaml` are re-read from disk on every switch, so an animation added after startup needs no restart. <br/> **_string:_** `car_wave` |
| ✅  | ❌  | `led_animation_user_dir` | ROSbot XL only. Extra directory scanned for `<name>.png` + `<name>.yaml`, overriding shipped animations by name. Empty follows `config_dir` to `<config_dir>/rosbot_utils/animations`, which is where `create_config_dir` puts them. <br/> **_string:_** `""` |
