...
- *2026-05-21: Added MAVLink alternative link layer on `jazzy-mavlink`. New arg `link_layer:=mavlink` on `rosbot[_xl].yaml` selects `mavlink_bridge.launch.py` instead of `microros.launch.py`. Renamed the legacy `microros` (bool) bringup arg to `hardware_bridge` since it now gates both protocols — snap/scripts must update `microros:=False` → `hardware_bridge:=False`. `configure_robot` gained `--expected-firmware` (default keeps `v1.1.0-jazzy`; the MAVLink launch passes `v0.1.1-jazzy-mavlink`). `flash_firmware` gained `--variant micro-ros|mavlink` to pick between bundled binaries (`rosbot[_xl]-v1.1.0-jazzy.bin` vs `rosbot[_xl]_mavlink-v0.1.1.bin`). Release tooling (`just release`, tag-driven workflow, CHANGELOG.md bootstrap) was added as infrastructure — **do not run releases on dev branches**; bare-suffix `X.Y.Z` tags belong on `jazzy`.*
- *2026-05-22: Wired `configure_robot` into the runtime-switch firmware handshake (`feat/runtime-comm-impl`). `--backend microros|mavlink` makes the script send the new `BACKEND:` line ahead of `NS:`; `microros.launch.py` and `mavlink.launch.py` now pass the matching value. A missing ACK is fatal: legacy single-protocol firmware silently falls out of the handshake on an unknown line and namespace would default, so we want loud failure — the error tells the operator to flash a runtime-switch build. Firmware-version pin in `--expected-firmware` is unchanged for now; bump it when the runtime-switch firmware lands a new release tag (and swap the corresponding `rosbot_utils/firmware/*.bin`).*
- *2026-05-22: Renamed the bringup arg `link_layer` → `backend` end-to-end (yamls + launches + ROS_API.md + README.md + ARCHITECTURE.md). The original name dates to the May 21 MAVLink addition; "link layer" is the OSI L2 abstraction, but what we're picking is the application-protocol framing (XRCE-DDS vs MAVLink) — `backend` matches the firmware-side terminology (`CommBackend` enum, `--backend` flag in `configure_robot`, `BACKEND:` handshake line) so the same word follows the value all the way from `snap set rosbot driver.backend=…` through the launch arg into the firmware. Breaking change for anyone passing `link_layer:=…` or reading the bringup args table — no compatibility alias.*
