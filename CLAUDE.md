...
> **Automation hint.** Most of the checklist below is wired into the `/feature` slash command (see [.claude/README.md](.claude/README.md) for layout). Lighter alternatives: `/quick-fix` for bug fix / small refactor (≤3 files, no public API change), `/spec` for spec drafting only (no implementation), `/review` for parallel optimizer+security+tester review on the current branch. The checklist below is still the source of truth — orchestrator follows it; humans verify it.
...
- *2026-05-21: Namespace audit (Phases 1-5 + 7 of [.claude/plans/namespace-audit.md](.claude/plans/namespace-audit.md)). **Decyzje:** Q1 → `/diagnostics` per-namespace (`controller_manager`, `ekf_node`, ros2_control plugins). Q2 → `controller_manager` services namespaced: HW keeps the `push_ros_namespace` group from bringup; sim requires explicit URDF `<remapping>` entries in [gazebo.urdf.xacro](rosbot_description/urdf/common/gazebo.urdf.xacro) because `gz_ros2_control` instantiates `controller_manager` inside the Gazebo process before any launch `GroupAction` is applied — the 14 entries cover the full public CM surface in `ros-jazzy-controller-manager` (11 services: `cleanup_controller`, `configure_controller`, `list_controllers`, `list_controller_types`, `list_hardware_components`, `list_hardware_interfaces`, `load_controller`, `reload_controller_libraries`, `set_hardware_component_state`, `switch_controller`, `unload_controller` + 3 diagnostic topics: `activity`, `statistics`, `introspection_data`). Q3 → policy is hard-coded (no `diagnostics_global` / `tf_global` runtime args; integrators override via `config_dir`). Q4 → sim/HW divergence of namespacing mechanisms is documented in [ARCHITECTURE.md](ARCHITECTURE.md) section 5. **micro_ros_agent** explicit `namespace=` was attempted as a defensive forward, **but HW retest (2026-05-21) revealed it produces double-stack** — agent process spawns with `__ns:=/test_ns/test_ns` because `push_ros_namespace` IS active inside the `RegisterEventHandler`/`OnProcessExit` callback, contrary to the original hypothesis. Reverted: micro_ros_agent `Node(...)` carries no `namespace=` kwarg. Empirical evidence: after revert, `ps -ef | grep micro_ros_agent` shows `__ns:=/test_ns` (single), `ros2 service list` shows zero leaks, firmware data topology under `/test_ns/` unchanged. **Lesson:** `OpaqueFunction` returning a Node from a `RegisterEventHandler` callback does NOT pop the parent group's launch_configurations — `push_ros_namespace` propagates correctly. Avoid the `dock.launch.py:38-41` "explicit forward" idiom unless the Node is constructed at top-level (outside any event-handler return). **Regression guards added:** [rosbot_bringup/test/test_namespace_isolation.py](rosbot_bringup/test/test_namespace_isolation.py) (offline base stack — fails if any topic/service leaks to `/` outside the global allow-list `{tf, tf_static, parameter_events, rosout}`); [rosbot_description/test/test_xacro.py::test_gazebo_urdf_namespace_remappings](rosbot_description/test/test_xacro.py) (asserts the 14 URDF `<remapping>` entries are present in the `gz_ros2_control` plugin block); [rosbot_bringup/test/test_launch_offline.py::test_microros_agent_node_has_no_explicit_namespace](rosbot_bringup/test/test_launch_offline.py) (AST scan that the `Node` action for `micro_ros_agent` does NOT pass `namespace=`, post HW retest revert). **HW retest results (Phase 6, 2026-05-21):** Live ROSbot XL + manipulator, `ROBOT_NAMESPACE=test_ns`, isolated `ROS_DOMAIN_ID=42`. Zero leaks — 250 services all under `/test_ns/`, only `/tf`, `/tf_static`, `/parameter_events`, `/rosout` on root. **Krok 2.1 (CM):** `push_ros_namespace` from bringup covers the full CM surface — all 27 entries under `/test_ns/controller_manager/*` including `cleanup_controller`, `activity/{full,names,values}`, `statistics/{full,names,values}`, `introspection_data/{full,names,values}`. No `controller.yaml` change needed for HW. **Krok 2.3 (EKF):** `robot_localization`'s `ekf_node` registers `set_pose`/`enable`/`toggle` as relative services, so push_ros_namespace lands them at `/<ns>/set_pose` (NOT `/<ns>/ekf_node/set_pose` — tester L4 hypothesis was wrong). `REQUIRED_NS_SERVICES` in `test_namespace_isolation.py` restored to include `/<ns>/set_pose`. **Krok 2.5 (MoveIt):** `move_group`, `servo_node`, planning services (`get_planning_scene`, `compute_ik`, `compute_fk`, `apply_planning_scene`, `check_state_validity`) all under `/test_ns/`. 2026-05-15 fixes hold. Snapshots: `.claude/scratch/namespace-audit-hw-snapshot{,-postrevert}.txt`.*
- *2026-05-21: Sim retest (`ros2 launch rosbot_gazebo simulation.yaml robot_model:=rosbot_xl namespace:=test_ns configuration:=manipulation headless:=True`, isolated `ROS_DOMAIN_ID=42`). Robot stack identycznie jak HW — 249 services (vs HW 250, off-by-one), 27 entries pod `/test_ns/controller_manager/*` (14 URDF `<remapping>` + 13 standard CM services), `/test_ns/{set_pose,enable,toggle}` EKF, `/test_ns/{get_planning_scene,compute_ik,compute_fk,apply_planning_scene,check_state_validity}` MoveIt, `/test_ns/diagnostics`. **2 expected sim-only globals poza scope namespace-audit:** (1) `/gz_bridge/*` (8 parameter services) — `parameter_bridge` w [simulation.yaml](rosbot_gazebo/launch/simulation.yaml) jest celowo poza `spawn_robot.yaml` push_ros_namespace bo hostuje globalny `/clock`; (2) `/launch_ros_*` (8 parameter services) — launch_ros framework internal node, artifact każdego launch'a, nie rosbot code. Snapshot: [namespace-audit-sim-snapshot.txt](.claude/scratch/namespace-audit-sim-snapshot.txt). **Caveat:** gazebo.urdf.xacro komentarz block musi być wolny od `:` — gz_sim's URDF→SDF konwerter parsuje przez yaml.safe_load i pada na "key: value" w XML komentarzach (`mapping values are not allowed here, line N column M`). Skrócony komentarz w xacro bez dwukropków — block delegowany do CLAUDE.md jako single source of truth.*
