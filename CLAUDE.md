...
- *2026-08-12 — `tf_namespace_bridge` moved from a standalone process (launched separately by `rosbot_bringup`/`rosbot_gazebo`) into `rosbot_description`'s `node_container`, composed with `robot_state_publisher` (`rclcpp_components/component_container_mt`). Rationale: RSP is the highest-frequency `/tf` publisher in the stack and the bridge is its direct subscriber, so intra-process delivery skips a serialize/DDS hop on every transform; also deduplicates 3 near-identical standalone-node launch blocks (bringup ROSbot, bringup XL, gazebo spawn) into one. `tf_namespace_bridge` (public bool arg) flows down as an ambient ROS launch configuration through `rosbot_controller/controller.yaml`'s include (no explicit re-forwarding needed — same mechanism `namespace`/`config_dir` already relied on). `tf_namespace_bridge_config` is an internal `let` in `rosbot_description`, resolved from `config_dir` like `components_config` — not a public arg. Since the config content was identical across the 3 old copies anyway, `rosbot_bringup/config/tf_namespace_bridge.yaml` and `rosbot_gazebo/config/tf_namespace_bridge.yaml` were deleted; `rosbot_description/config/tf_namespace_bridge.yaml` is now the single owner (override via `config_dir` same as any other package config). **Gotcha found while verifying:** any `node_container`/`composable_node` use makes `launch_ros` spawn one unnamespaced helper node `launch_ros_<pid>` (hardcoded in `launch_ros/ros_adapters.py`'s `ROSAdapter`, no YAML/namespace hook) to call the container's `load_node` service — it only exposes empty parameter services and lives for the launch process's lifetime, but it trips the namespace-isolation "zero global leaks" guard. No upstream `ros2/launch_ros` issue tracks this (checked, 2026-08); allowlisted by prefix (`ALLOWED_GLOBAL_SERVICE_PREFIXES = {"/launch_ros_"}`) in [test_namespace_isolation.py](rosbot_bringup/test/test_namespace_isolation.py) with this rationale — accept the same prefix for any future `node_container` additions rather than re-litigating. HW/sim-verified via isolated symlink workspace (real Gazebo run still pending — do before merge).*
