...
Status: **implemented, pending on-hardware build/test** (2026-07-05). Code landed in `rosbot_utils` (`src/animation_publisher.cpp`, `third_party/stb_image.h`, `animations/`), `rosbot_bringup/launch/rosbot_xl.yaml`, `rosbot_utils/test/test_led_strip.py`, and the `rosbot-snap` `driver.current-animation` wiring. Node compiles against ROS Jazzy + PNG decode verified on-host; a full colcon build + snap rebuild + on-robot run are the remaining validation. Delete this SPEC once validated and migrate any durable notes into `ARCHITECTURE.md` / the READMEs.
